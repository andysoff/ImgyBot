...
### 🔴 Критическое напоминание: не катить на прод без команды

❗ **Ничего не деплоить на прод без явной команды Андрея «кати в прод» (или аналога).**

Даже если кажется, что правка очевидная и безопасная — прод только с команды.
- Правка → коммит → тест (только тест!)
- Тестовый бот: `systemctl restart avatar-bot-test`
- Прод-бот: только после «кати в прод»
- Спросил — жди ответа. Не делай ничего, пока не ответили.

Я знаю, что я склонен забывать это правило. Оно здесь, чтобы я прочитал его в следующий раз.

### 🔄 Рестарт бота после правок (тест)

После любых изменений кода бота (bot-logic.js, bot-runner.js, generate-image.js и т.д.):
1. Правка → коммит
2. **Только тестовый бот:** `systemctl restart avatar-bot-test`
3. Прод не трогать до команды Андрея
...
- Скрипт `deploy/deploy-prod.sh` проверяет файл-триггер `/tmp/imgy-prod-deploy-ok`
- Пока триггера нет — скрипт отказывается деплоить
- Триггер создаётся ТОЛЬКО после фразы Андрея «катай в прод» (или аналогичной)
- После успешного деплоя триггер удаляется — каждый раз нужно новое разрешение
- Прямое копирование в `/opt/avatar-service-prod/` без триггера — тоже нарушение

## 📋 Чеклист добавления нового стиля

При добавлении нового стиля или подстиля нужно обновлять **все места**, а не только `styles.json`:

1. **`styles.json`** — структура меню (группы, подстили, названия)
2. **`STYLE_PROMPTS`** в `generate-image.js` — промпты для Gemini (без них нейросеть не знает, что генерировать)
3. **`STYLE_PROMPTS_FEMALE`** в `generate-image.js` — женские версии промптов (если нужны)
4. **Деплой:** синхронизировать все изменённые файлы: тест ↔ гит ↔ прод

Если меняю любой из этих файлов — **автоматом проверяю, не нужно ли обновить остальные**. Не надеяться на память, пройти по чеклисту.

### Порядок синхронизации изменений

Когда правлю файлы на тесте (в `/opt/avatar-service-test/`):
1. Сразу копирую изменения в гит (`avatar-service/`)
2. Сразу копирую изменения в прод (`/opt/avatar-service-prod/`), если это не data-файлы (они свои у каждой среды)
3. Фиксирую в гите
4. Рестартую тест и/или прод

Для data-файлов (не версионируются):
- `data/styles.json` — копирую отдельно в `/opt/avatar-service-prod/data/`
- `data/users.json`, `data/avatars.json` — не трогаю, они персональные
- `data/*.json` с контентом стилей — копирую на прод руками
